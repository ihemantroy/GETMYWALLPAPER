...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { id, original, width, height, photographer, photographer_url, alt, categoryId } = b;
    ...
    const path = `import/pexels-${id}.${ext}`;
    ...
    const { error: insErr } = await admin.from("wallpapers").insert({
      ...
      credit_url: photographer_url || `https://www.pexels.com/photo/${id}`,
      tags: [],
      status: "published",
      is_community: false,
